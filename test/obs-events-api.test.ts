...
/**
 * GET /tasks/:id/events —— 在途事件流的读端点。
 *
 * 这个端点存在的全部理由是:**任务 RUNNING 时就读得到事件**。`/admin/events` 做不到,
...
describe("GET /tasks/:id/events", () => {
  it("任务 RUNNING 即可读,内容 entirely 来自 R2:D1 归档表一条事件都没有", async () => {
    ...
    const { status, body } = await eventsBody(`/tasks/${taskId}/events`);
  ...
  it("信封就是承诺的 8 个字段:不建 hash chain,也不外带 transcript 原文", async () => {
    ...
    const { body } = await eventsBody(`/tasks/${taskId}/events`);
  ...
  it("白名单与凭据打码在 ingress 已完成:端点文本里没有 key 原文", async () => {
    ...
    const text = await (await request(`/tasks/${taskId}/events`)).text();
  ...
  it("?after/?limit 逐页翻等于一次读全,跨 attempt 边界不断序", async () => {
    ...
    const p1 = await eventsBody(`/tasks/${taskId}/events?limit=2`);
    expect(p1.body.events.map((e) => e.seq)).toEqual([1, 2]);
    expect(p1.body.next_cursor).toBe(2);

    const p2 = await eventsBody(`/tasks/${taskId}/events?after=2&limit=2`);
  ...
  it("?after 跨定长段边界时按 index 的段清单定位,不错位也不漏读", async () => {
    ...
    const edge = await eventsBody(`/tasks/${taskId}/events?after=199&limit=3`);
    expect(edge.body.events.map((e) => e.seq)).toEqual([200, 201, 202]);
    expect(edge.body.next_cursor).toBe(202);

    const tail = await eventsBody(`/tasks/${taskId}/events?after=249`);
    ...
    const beyond = await eventsBody(`/tasks/${taskId}/events?after=250`);
  ...
  it("limit 缺省 500、上限 2000;越界与非数字 → 400", async () => {
    ...
    expect((await eventsBody(`/tasks/${taskId}/events`)).body.count).toBe(2);
    expect((await eventsBody(`/tasks/${taskId}/events?limit=2000`)).status).toBe(200);
    for (const limit of ["0", "-1", "2001", "abc", "1.5", "", "1e400"]) {
      const res = await request(`/tasks/${taskId}/events?limit=${encodeURIComponent(limit)}`);
  ...
  it("after 只接受非负整数", async () => {
    ...
    for (const after of ["-1", "1.5", "abc", "", "1e400"]) {
      const res = await request(`/tasks/${taskId}/events?after=${encodeURIComponent(after)}`);
    ...
    expect((await eventsBody(`/tasks/${taskId}/events?after=0`)).status).toBe(200);
  });
  ...
  it("任务不存在 → 404", async () => {
    const res = await request(`/tasks/${crypto.randomUUID()}/events`);
    expect(res.status).toBe(404);
    expect(((await res.json()) as ErrorBody).error.type).toBe("not_found");
  });

  it("鉴权与 GET /tasks/:id 同一条 checkApiToken 路径", async () => {
    expect(TOKEN).toBeTruthy();
    const { taskId } = await seedTaskWithEvents([1]);
    expect((await request(`/tasks/${taskId}/events`, null)).status, "缺 token 应 401").toBe(401);
    expect((await request(`/tasks/${taskId}/events`, "wrong-token")).status, "错 token 应 401").toBe(401);
    expect((await request(`/tasks/${taskId}/events`)).status).toBe(200);
  });
  ...
  it("某 attempt 的 journal 读坏了:其余照常返回,坏的那个如实列出", async () => {
    ...
    const res = await request(`/tasks/${taskId}/events`);
  ...
  it("落地页登记该端点,并如实说明它读 R2、RUNNING 即可用", async () => {
    ...
    expect(html).toContain("<dt>GET /tasks/:id/events</dt>");
