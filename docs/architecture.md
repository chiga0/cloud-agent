...
| **评审材料与喂入 prompt 同源** | 【原始任务】+【验收标准】块经 `reviewTaskSection` 唯一组装、两处消费:prompt 模板内嵌的就是 `material.task_prompt` 原文(逐字同串),并入后才按 `MATERIAL_LIMITS.task_prompt` 截断。w4a 教训(§N.38):标准块只进 prompt 不进材料时,诚实引用标准原文的 reject 在结构上必然 quote_not_found。材料组成变更会让 `material_digest` 随之变化(纯观察面,无路由分支依赖其具体值);reviewer 闸门 shadow/enforce 模式语义不变 | `review.requested`(material_digest) |
...
| **空候选在事件链上明说(仅观察,不是闸门)** | writer 成功回报的 `candidate` 等于空串 sha256(`e3b0c44…`)即记一条观察事件:全历史普查 3/144 例全是 writer 自报成功的假成功形状(§N.37)。它无否决权、不路由——首个空 patch 仍由 verify 拦下(git apply 对空输入 exit 128),重复空候选仍走 no_progress 熔断转人工 | `candidate.empty_patch` |
