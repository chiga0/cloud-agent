 ...
 * 是真实成本:`Sandbox rpc:readFile` 单次 wall 实测 30–33 秒(tail 里 26 次全部如此),
 * 一轮 = 30s 睡眠 + 一次 30 秒级的读。**「摄取节拍每 30s 一次」这个前提就是这么被证伪的**
 * —— 旧阈值 90/300 全都建立在它之上。
 */
 ...
 * 最长 94s = 连跳 2 轮。心跳上线后这个形状会直接以 `gap_ms` 出现在 journal 里,
 * 不必再靠 tail 反推。导出它是为了让测试能钉住「红线 / 实测最坏间隔」这个比值 ——
 * 阈值与数据的比值是这条判据唯一可被后来者复查的凭据。
 */
 ...
 *
 * 为什么不取「一个最坏轮次 + 一点余量」(旧 stall 的写法):实测 22% 的轮次会被跳过,
